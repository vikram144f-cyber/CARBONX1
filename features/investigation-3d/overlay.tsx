...
export function Investigation3DOverlay({
  ...
  if (webgl === null) return <div className="absolute inset-0 z-50 grid place-items-center bg-cx-bg text-sm text-slate-400">Preparing investigation canvas…</div>;
  const fallback = <Investigation2DFallback data={data} onClose={onClose} />;
  return (
    <div className="fixed inset-0 z-50 bg-cx-bg">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between gap-4 p-4 sm:p-7">
        <div className="cx-surface-elevated pointer-events-auto max-w-md rounded-2xl p-4 backdrop-blur-sm">
        ...
        <div className="pointer-events-auto flex gap-2">
          ...
          <button type="button" onClick={onClose} className="cx-surface-elevated rounded-lg px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-white/10">Exit 3D</button>
        </div>
      </div>
