...
### Acceptance Criteria
- [x] Write a simple Solidity contract `CarbonXAnchor` that accepts an incident ID, a bytes32 hash, and an event type string.
...
### Acceptance Criteria
- [x] Build a canonical JSON object from the `RiskAssessment` and `Incident` state.
- [x] Ensure serialization is deterministic (e.g., using `JSON.stringify` with sorted keys).
- [x] Calculate the `keccak256` hash of the serialized string using `viem`.
- [x] The supported anchor event types must use the exact string literals: `"UNDER_ASSESSMENT"`, `"AUDIT_RECOMMENDED"`, and `"RESOLVED"`.
...
### Acceptance Criteria
- [x] Create a `BlockchainAnchor` record in PostgreSQL with status `PENDING`.
...
- [x] If the RPC fails or times out, update the status to `FAILED`.
- [x] Crucially, if the anchor fails, the incident state transition itself still succeeds and the workflow continues gracefully.
